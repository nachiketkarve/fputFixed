...
int main(int argc, char *argv[])
{
    ...
    if (model == "alpha")
    {
        FileName = saveFolder + "alphaCorrInit-N" + std::to_string(N) + "-K" + std::to_string(int(k0)) + "-E" + std::to_string(E0) + "-A" + std::to_string(nonLin) + ".csv";
    } else if (model == "toda")
    {
        FileName = saveFolder + "todaCorrInit-N" + std::to_string(N) + "-K" + std::to_string(int(k0)) + "-E" + std::to_string(E0) + "-A" + std::to_string(nonLin) + ".csv";
    } else if (model == "beta")
    {
        FileName = saveFolder + "betaCorrInit-N" + std::to_string(N) + "-K" + std::to_string(int(k0)) + "-E" + std::to_string(E0) + "-B" + std::to_string(nonLin) + ".csv";
    }

    std::ofstream file;
    file.open(FileName);
    file << std::setprecision(15);
    file << "Time" << "," << "Correlation";

    Vector dH(int(tmax/deltaT));
    Vector Corr(int(tmax/deltaT));
    Vector CorrAvg(int(tmax/deltaT));
    ...
    for (int average = 0; average < averages; average++)
    {
        ...
        {
            for(int j = 0; j < t.size() - i; j = j + iStep)
            {
                Corr(i) = Corr(i) + dH(j)*dH(j+i)/(t.size()-i);
            }
        }

        CorrAvg = CorrAvg + Corr/averages;
    }
