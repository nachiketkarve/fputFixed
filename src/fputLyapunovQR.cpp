        ...
        for (int i = 0; i < int(tmax/deltaT); i++)
        {
            ...
            if (i % iStep == 0)
            {
                if (currT == 0){
                    ...
                    Eigen::HouseholderQR<Eigen::MatrixXd> qr_no_pivoting(Phi);
                    Eigen::MatrixXd Q_no_pivoting = qr_no_pivoting.householderQ();
                    Eigen::MatrixXd R_no_pivoting = qr_no_pivoting.matrixQR().triangularView<Eigen::Upper>();
                    ...
                    double maxEigVal = R(0,0);
                    for (int eigs = 1; eigs < 2*N; eigs++)
                    {
                        if (R(eigs,eigs) > maxEigVal)
                        {
                            maxEigVal = R(eigs,eigs);
                        }
                    }
                    Lya(i) = Lya(i) + std::log(maxEigVal) / (currT * averages);
                }    
