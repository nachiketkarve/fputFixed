...
int main(int argc, char *argv[])
{
    ...
    for (int average = 0; average < averages; average++)
    {
        ...
        Matrix qTangent(N+2, 1);
        Matrix pTangent(N+2, 1);
        ...
        for (int i = 0; i < int(tmax/deltaT); i++)
        {
            ...
            if (i % iStep == 0)
            {
                if (currT == 0){
                    ...
                    for (int j = 0; j < N + 2; j++)
                    {
                        norm = norm + qTangent(j, 0) * qTangent(j, 0) + pTangent(j, 0) * pTangent(j, 0);
                    }

                    norm = std::sqrt(norm);

                    ratio = ratio * norm;
                    qTangent = qTangent / norm;
                    pTangent = pTangent / norm;

                    Lya(i) = Lya(i) + std::log(ratio) / (currT * averages);
                }    
