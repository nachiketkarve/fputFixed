...
namespace todaFixed
{
    ...
    double Coupling4(int i, int j, int k, int l, int N){
        double D = 0;
        ...
        D = D*freq(i,N)*freq(j,N)*freq(k,N)*freq(l,N)/double(2.0*(N+1));
    ...
    {
        double V0 = 1/(4.0*alpha*alpha);
        double lmd0 = 2.0*alpha;
        double Etot = 0;
        int N = q.size() - 2;
        ...
        for (int i = 0; i < N + 2; i++)
        {
            ...
            if (i < N + 1)
            {
                Etot = Etot + V0*(std::exp(lmd0*(q(i+1)-q(i))) - 1.0 - lmd0*(q(i+1)-q(i)));
            }
        }
    ...
    double EnergyNormalSpace(const Vector &Q, const Vector &P, double alpha)
    {
        ...
        double beta = 2.0*alpha*alpha/3.0;
        ...
        for(int i = 0; i < N+2; i++){
            for(int j = 0; j < N+2; j++){
                for(int k = 0; k < N+2; k++){
                    for(int l = 0; l < N+2; l++){
                        Etot = Etot + beta / 4.0 * Coupling4(i, j, k, l, N) * Q(i) * Q(j) * Q(k) * Q(l);
                    }
                }
            }
        }
    ...
    void EOMe(Vector &ddq, Matrix &ddqe, const Vector &q, const Matrix &qe, double alpha)
    {
        ...
        for (int i = 1; i <= N; i++)
        {
            ...
            {
                ddqe(i, j) = std::exp(lmd0*(q(i+1)-q(i)))*(qe(i+1,j)-qe(i,j)) - std::exp(lmd0*(q(i)-q(i-1)))*(qe(i,j)-qe(i-1,j));
            }
        }
    }
    ...
    void ModeAmplitude(Vector &Q, const Vector &P, double k0, double E0, double alpha)
    {
        ...
        for (int i = 0; i < N + 2; i++)
        {
            for (int j = 0; j < N + 2; j++)
            {
                ...
                coeff(2) = coeff(2) + 6.0/4.0*beta*Coupling4(i,j,int(k0),int(k0),N)*Q(i)*Q(j);
            }
        }
        for(int i = 0; i < N+2; i++){
            for(int j = 0; j < N+2; j++){
                for(int k = 0; k < N+2; k++){
                    coeff(1) = coeff(1) + beta*Coupling4(i,j,k,int(k0),N)*Q(i)*Q(j)*Q(k);
                }
            }
        }
        ...
        for (int i = 0; i < N + 2; i++)
        {
            ...
            coeff(3) = coeff(3) + beta*Coupling4(i,int(k0),int(k0),int(k0),N)*Q(i);
        }
        coeff(3) = coeff(3) + alpha / 3.0 * Coupling(int(k0), int(k0), int(k0),N);
        coeff(4) = coeff(4) + beta/4.0*Coupling4(int(k0),int(k0),int(k0),int(k0),N);
