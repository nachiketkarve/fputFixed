...
#include "fputBetaFixed.h"
#include "eigen/Eigen/Eigenvalues"
#include <fstream>
#include <string>
#include <complex>

using namespace fputBetaFixed;
...
int main()
{
    ...
    std::cin >> k0;
    double beta = 0.01;
    std::cout << "Enter Initial beta ";
    std::cin >> beta;
    double deltabeta = 0.01;
    std::cout<<"Enter delta beta ";
    std::cin>>deltabeta;
    double betaMax = 1.0;
    std::cout << "Enter Max beta ";
    std::cin >> betaMax;
    ...
    while (beta <= betaMax)
    {
        ...
        double period = 0;
        period = PoincareSection_rk(Z, k0, E0, beta, tmax, deltaT);
        std::cout << period << "\n";

        Matrix QeM(N + 2, N + 2);
        Matrix PeM(N + 2, N + 2);
        PerturbationMatrix(QeM, PeM, Q0, k0, beta);
        PoincareSectione_rk(QeM, PeM, Q0, k0, E0, beta, tmax, deltaT);
        ...
        Push(Qn, Zn, k0, E0, beta);

        Vector Znn(Z.size());
        Znn = Zn;
        PoincareSection_rk(Znn, k0, E0, beta, tmax, deltaT);
        Vector Qnn(N + 2);
        Vector qnn(N + 2);
        Vector pnn(N + 2);
        Push(Qnn, Znn, k0, E0, beta);
        qnn = FourierComponents * Qnn;
        pnn.setZero();
        double lmd = Mod(Qnn - Qn) / Mod(Qn);
        std::cout << "Energy = " << TotalEnergy(qnn, pnn, beta) << " lmd = " << lmd << "\n";
        Q0 = Qn;
        
        if(lmd > pow(10,-1) || count > 15 || std::isnan(lmd) || std::isnan(TotalEnergy(qnn, pnn, beta))){
            count = 0;
            Q0 = Qb;
            E0 = E0b;
            beta = beta + deltabeta;
        ...
        if (lmd < pow(10, -12) && lmd > 0)
        {
           ...
           period = PoincareSectione_rk(QeFull, PeFull, Q0, k0, E0, beta, tmax, deltaT);
            ...
            */
            std::ofstream file;
            FileName = "Fixed-N" + std::to_string(N) + "-K" + std::to_string(int(k0)) + "-E" + std::to_string(E0) + "-B" + std::to_string(beta) + ".csv";
            file.open(FileName);
            file<<std::setprecision(15);
            file<<"Q"<<","<<"EigValReal"<<","<<"EigValImag"<<","<<"N"<<","<<"beta"<<","<<"k"<<","<<"E0"<<","<<"lmd"<<","<<"period"<<"\n";
            file<<Qn[0]<<","<<std::real(eigVals(0))<<","<<std::imag(eigVals(0))<<","<<N<<","<<beta<<","<<k0<<","<<E0<<","<<lmd<<","<<period<<"\n";
